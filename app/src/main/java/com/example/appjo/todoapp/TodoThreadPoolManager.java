...
public class TodoThreadPoolManager {

    private static TodoThreadPoolManager mInstance = null;
    ...
    private WeakReference<MessageUpdateCallback> messageUpdateCallbackWeakReference;

    static {
        KEEP_ALIVE_TIME_UNIT = TimeUnit.SECONDS;
        mInstance = new TodoThreadPoolManager();
    }

    private TodoThreadPoolManager() {
        // initialize a queue for the thread pool. New tasks will be added to this queue
        mTaskQueue = new LinkedBlockingQueue<Runnable>();

        mRunningTaskList = new ArrayList<>();

        Log.e(UtilClass.LOG_TAG,"Available cores: " + NUMBER_OF_CORES);

         /*
            Inorder to choose from fixed and dynamic size thread pool choose one and comment another
         */
    ...
    public static TodoThreadPoolManager getmInstance() {
        return mInstance;
    }
    ...
    public void cancelAllTasks() {
        ...
        sendMessageToUiThread(UtilClass.createMessage(UtilClass.MESSAGE_ID, "All tasks in the thread pool are cancelled"));
    }

    // Keep a weak reference to the UI thread, so we can send messages to the UI thread
    public void setUiThreadCallback(MessageUpdateCallback messageUpdateCallback) {
        this.messageUpdateCallbackWeakReference = new WeakReference<MessageUpdateCallback>(messageUpdateCallback);
    }

    // Pass the message to the UI thread
    public void sendMessageToUiThread(Message message){
        if(messageUpdateCallbackWeakReference != null && messageUpdateCallbackWeakReference.get() != null) {
            messageUpdateCallbackWeakReference.get().publishToUiThread(message);
        }
    }
        ...
        public Thread newThread(Runnable runnable) {
            ...
            thread.setUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                ...
                public void uncaughtException(Thread thread, Throwable ex) {
                    Log.e(UtilClass.LOG_TAG, thread.getName() + " encountered an error: " + ex.getMessage());
                }
            });
            return thread;
        }
